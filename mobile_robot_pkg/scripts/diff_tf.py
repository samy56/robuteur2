...
#this code has been simplified by me 
class DiffTf:
	def __init__(self):
	#############################################################################
	...
	#Wheel radius : 0.0325 
		# wheel circum =    = 0.2041
	# One rotation encoder ticks : 20 ticks
	# For 1 meter: 20 * ( 1 / 0.2041) = 98 ticks

		self.rate = rospy.get_param('~rate',50.0)  # the rate at which to publish the transform
		self.ticks_meter = float(rospy.get_param('ticks_meter', 20))  # The number of wheel encoder ticks per meter of travel
